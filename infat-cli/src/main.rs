...
async fn main() -> Result<()> {
    ...
    color_eyre::install()
        .wrap_err("Failed to install color-eyre error handler")?;

    let cli = Cli::parse();
    let global_opts: GlobalOptions = (&cli).into();
    
    // Initialize tracing
    infat_lib::init_tracing(&global_opts)
    ...
    match cli.command {
        None => {
            ...
            handle_config_load(&global_opts).await
                .wrap_err("Failed to load and apply configuration")?;
        }
        Some(Commands::Info { app, ext, r#type }) => {
            handle_info_command(app, ext, r#type).await
                .wrap_err("Info command failed")?;
        }
        Some(Commands::Set { app_name, ext, scheme, r#type }) => {
            handle_set_command(&global_opts, app_name, ext, scheme, r#type).await
                .wrap_err("Set command failed")?;
        }
        Some(Commands::Init { output }) => {
            handle_init_command(&global_opts, output).await
...
async fn handle_config_load(opts: &GlobalOptions) -> Result<()> {
    ...
    if !opts.quiet {
        println!("📄 Loading configuration from: {}", config_path.display().bright_cyan());
    }
    ...
    // Apply configuration
    config::apply_config(&config, opts.robust).await
...
async fn handle_info_command(
    ...
    if let Some(app_name) = app {
        ...
        if !app_info.declared_types.is_empty() {
            ...
            for declared_type in &app_info.declared_types {
                ...
                if !declared_type.extensions.is_empty() {
                    let exts: Vec<String> = declared_type.extensions.iter()
                        .map(|ext| format!(".{}", ext))
                        .collect();
        ...
        println!("📄 File Extension: {}", format!(".{}", extension).bright_green());
...
async fn handle_set_command(
    ...
    if let Some(extension) = ext {
        ...
        association::set_default_app_for_extension(&extension, &app_name).await
            .wrap_err_with(|| format!("Failed to set default app for .{}", extension))?;
        
        if !opts.quiet {
            println!("{} Set .{} → {}", "✓".bright_green(), extension, app_name.bright_cyan());
        }
        
    } else if let Some(url_scheme) = scheme {
        info!("Setting {} as default for {} scheme", app_name, url_scheme);
        
        association::set_default_app_for_url_scheme(&url_scheme, &app_name).await
            .wrap_err_with(|| format!("Failed to set default app for {} scheme", url_scheme))?;
        
        if !opts.quiet {
            println!("{} Set {} → {}", "✓".bright_green(), url_scheme, app_name.bright_cyan());
        }
        
    } else if let Some(type_name) = r#type {
        info!("Setting {} as default for type {}", app_name, type_name);
        
        association::set_default_app_for_type(&type_name, &app_name).await
            .wrap_err_with(|| format!("Failed to set default app for type {}", type_name))?;
        
        if !opts.quiet {
            println!("{} Set type {} → {}", "✓".bright_green(), type_name, app_name.bright_cyan());
        }
    }
...
async fn handle_init_command(opts: &GlobalOptions, output: Option<PathBuf>) -> Result<()> {
    ...
    if !opts.quiet {
        println!("💾 Writing configuration to: {}", output_path.display().bright_cyan());
    }
